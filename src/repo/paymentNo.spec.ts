...
describe('購入番号を発行する', () => {
    ...
    it('redisが正常であれば数字のみで構成された文字列を取得できるはず', async () => {
        ...
        const paymentNoRespo = new ttts.repository.PaymentNo(redisClient);
    ...
    it('redisでエラーが発生すれば、そのままエラーとなるはず', async () => {
        ...
        const result = await paymentNoRespo.publish(date).catch((err) => err);
    ...
    it('日付フォーマットが適切でなければArgumentエラー', async () => {
        ...
        const paymentNoRespo = new ttts.repository.PaymentNo(redisClient);
        sandbox.mock(redisClient).expects('multi').never();
...
describe('購入番号のためのチェックディジットを求める', () => {
    ...
    it('1桁の数字を取得できるはず', async () => {
        const source = '1'.repeat(ttts.repository.PaymentNo.MAX_LENGTH_OF_SEQUENCE_NO);
        const result = ttts.repository.PaymentNo.GET_CHECK_DIGIT(source);
    ...
    it('対象文字列の長さが適切でなければArgumentエラー', async () => {
        const source = '1'.repeat(ttts.repository.PaymentNo.MAX_LENGTH_OF_SEQUENCE_NO - 1);
        assert.throws(
            () => {
                ttts.repository.PaymentNo.GET_CHECK_DIGIT(source);
            },
            ttts.factory.errors.Argument
        );
        sandbox.verify();
    });
});
...
describe('購入番号の有効性チェック', () => {
    ...
    it('有効であればtrueが返るはず', async () => {
        ...
        const result = ttts.repository.PaymentNo.VALIDATE(paymentNo);
        assert(result === true);
        sandbox.verify();
    });

    it('購入番号の長さが適切でなければfalse', async () => {
        const paymentNo = '1'.repeat(ttts.repository.PaymentNo.MAX_LENGTH_OF_SEQUENCE_NO);
        const result = ttts.repository.PaymentNo.VALIDATE(paymentNo);
        assert(result === false);
        sandbox.verify();
    });
