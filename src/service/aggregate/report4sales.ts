 ...
 */
export function aggregateSales(targetDate: string) {
    return async (
        ...
        const cancels = await returnOrderTransactions2cancelDatas(returnOrderTransactions, reservationRepo, transactionEndDateBucket);
...
async function placeOrderTransactions2reservationDatas(
    ...
    for (const transaction of transactions) {
        ...
        eventReservations.forEach((r) => {
            datas.push(reservation2data(r, transactionResult.order.price, targetDate));
        });
    }
    debug('datas created.');
 ...
 */
function reservation2data(r: factory.reservation.event.IReservation, orderPrice: number, targetDate: Date): IData {
    ...
    return {
        ...
        cancellationFee: 0,
        transaction_endDate_bucket: targetDate
    };
}
...
async function returnOrderTransactions2cancelDatas(
    ...
    transactions.forEach((returnOrderTransaction) => {
        ...
        for (const r of eventReservations) {
            // 座席分のキャンセルデータ
            datas.push({
                ...reservation2data(r, placeOrderTransactionResult.order.price, targetDate),
