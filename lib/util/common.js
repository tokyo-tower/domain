 ...
 * @returns {any}
 */
function parseFromKeys(model, keys) {
    const copiedModel = {};
    // 指定キーの項目のみコピー
...
exports.parseFromKeys = parseFromKeys;
/**
 * 指定キーを削除したオブジェクト取得
 * 2017/06 add for TTTS
 *
 ...
 * @returns {any}
 */
function deleteFromKeys(model, keys) {
    const deletedModel = {};
    // 削除指定キーの項目以外はコピー
    Object.getOwnPropertyNames(model).forEach((propertyName) => {
        if (keys.indexOf(propertyName) < 0) {
...
exports.deleteFromKeys = deleteFromKeys;
/**
 * token取得
